...
export async function openRealtimeSession({
  ...
  timeoutMs = 2500,
  webTransportEligible = location.protocol === "https:",
  transportPreference = "auto",
  webTransportUrl,
  crypto = null,
}) {
  if (shouldTryWebTransport(transportPreference, webTransportEligible)) {
    try {
      return await WebTransportSession.connect(webTransportUrl, { crypto, timeoutMs });
    } catch (error) {
      onWebTransportFallback?.(error);
    }
  }

  return WebSocketSession.connect({ controlWebSocketUrl, crypto, frameWebSocketUrl });
}

function shouldTryWebTransport(transportPreference, webTransportEligible) {
  ...
  return transportPreference === "webtransport" || transportPreference === "wt" || webTransportEligible;
}
...
class WebSocketSession extends RealtimeSession {
  ...
  static async connect({ controlWebSocketUrl, frameWebSocketUrl, crypto = null }) {
    let control;
    let frame;
    try {
      const controlPromise = WebSocketChannel.connect(controlWebSocketUrl, { crypto }).then((channel) => {
        control = channel;
        return channel;
      });
      const framePromise = WebSocketChannel.connect(frameWebSocketUrl, { binaryType: "arraybuffer", crypto }).then((channel) => {
        frame = channel;
        return channel;
      });
...
class WebSocketChannel extends RealtimeChannel {
  constructor(socket, { crypto = null } = {}) {
    super("websocket");
    this.crypto = crypto;
  ...
  static connect(url, { binaryType, crypto = null } = {}) {
    return new Promise((resolve, reject) => {
      ...
      const channel = new WebSocketChannel(socket, { crypto });
      let opened = false;
      let settled = false;
  ...
  send(data) {
    this.sendQueue = this.sendQueue.then(() => this.sendEncrypted(data)).catch(() => {
      this.dispatchError();
      this.close();
    });
  }
  ...
  async handleMessage(data) {
    try {
      if (this.crypto && typeof data === "string") {
        this.dispatchMessage(await this.crypto.decryptText(data));
        return;
      }
      if (this.crypto && typeof data !== "string") {
        this.dispatchMessage(await this.crypto.decryptBytes(data));
        return;
      }
  ...
  async sendEncrypted(data) {
    try {
      if (this.crypto && typeof data === "string") {
        this.socket.send(await this.crypto.encryptText(data));
        return;
      }
      if (this.crypto && typeof data !== "string") {
        this.socket.send(await this.crypto.encryptBytes(data));
        return;
      }
...
class WebTransportSession extends RealtimeSession {
  constructor(transport, { crypto = null } = {}) {
    super("webtransport");
    this.control = null;
    this.crypto = crypto;
    this.frame = new WebTransportFrameChannel(this, { crypto });
    this.transport = transport;
  }

  static async connect(url, { crypto = null, timeoutMs }) {
    const transport = new WebTransport(url, {
      congestionControl: "low-latency",
      requireUnreliable: false,
    });
    const session = new WebTransportSession(transport, { crypto });
  ...
  async openControlStream() {
    ...
    this.control = new WebTransportControlChannel(this, stream.writable.getWriter(), { crypto: this.crypto });
...
class WebTransportControlChannel extends RealtimeChannel {
  constructor(session, writer, { crypto = null } = {}) {
    super("webtransport");
    this.crypto = crypto;
    ...
    try {
      for (;;) {
        ...
        while (buffer.byteLength >= 4) {
          ...
          this.dispatchMessage(this.crypto ? await this.crypto.decryptText(text) : text);
        }
      }
    } catch {
  ...
  send(data) {
    ...
    this.writeQueue = this.writeQueue.then(async () => {
      const outbound =
        this.crypto && typeof data === "string" ? await this.crypto.encryptText(data) : data;
      const payload = typeof outbound === "string" ? TEXT_ENCODER.encode(outbound) : bytesFromStreamValue(outbound);
...
class WebTransportFrameChannel extends RealtimeChannel {
  constructor(session, { crypto = null } = {}) {
    super("webtransport");
    this.crypto = crypto;
  ...
  async handleDatagram(value) {
    const bytes = bytesFromStreamValue(value);
    const packet = parseDatagramFramePacket(bytes);
    if (!packet) {
      this.dispatchFrame({ data: await this.decryptFrameBytes(bytes) });
      return;
    }
    ...
    if (packet.chunkCount <= 1) {
      this.dispatchFrame({ data: await this.decryptFrameBytes(packet.payload), frameId: packet.frameId });
      return;
    }
    ...
    if (pending.received === pending.chunks.length) {
      ...
      this.dispatchFrame({
        data: await this.decryptFrameBytes(concatByteList(pending.chunks, pending.receivedBytes)),
  ...
  async handleStreamFrame(data) {
    ...
    if (!packet) {
      this.dispatchFrame({ data: await this.decryptFrameBytes(data) });
      return;
    }

    this.dispatchFrame({
      ...packet,
      data: await this.decryptFrameBytes(packet.data),
    });
  }

  async decryptFrameBytes(data) {
    if (!this.crypto) {
      return data instanceof ArrayBuffer ? data : arrayBufferFromBytes(bytesFromStreamValue(data));
    }
    return this.crypto.decryptBytes(data);
  }
  ...
  async readFrameStream(readable) {
    try {
      ...
      if (data.byteLength > 0) {
        await this.handleStreamFrame(data);
      }
    } catch {
      this.session.close();
    }
